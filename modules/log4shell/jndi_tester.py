...
class Log4ShellTester:
            ...
            for payload_info in payloads:
                ...
                if result:
                    vulnerabilities.append({
                        ...
                        "evidence": result,
                        "description": (
                            f"Potentially vulnerable - unconfirmed. JNDI injection payload in "
                            f"'{header_name}' header (variant: {variant}) produced a response "
                            f"containing a definitive Log4j stack trace. This is NOT confirmed — "
                            f"a true positive requires an out-of-band (OOB) callback server to "
                            f"observe the JNDI lookup reaching an external service."
                        ),
                        "remediation": (
            ...
            for payload_info in payloads[:3]:  # Test top 3 variants per param
                ...
                if result:
                    vulnerabilities.append({
                        ...
                        "evidence": result,
                        "description": (
                            f"Potentially vulnerable - unconfirmed. JNDI lookup payload ({variant}) "
                            f"via query parameter '{param_name}' produced a definitive Log4j stack "
                            f"trace. This is NOT confirmed — requires verification via an OOB "
                            f"callback server to detect the JNDI lookup reaching an external service."
                        ),
                        "remediation": (
            ...
            for payload_info in payloads[:2]:  # Top 2 variants per field
                ...
                if result:
                    vulnerabilities.append({
                        ...
                        "evidence": result,
                        "description": (
                            f"Potentially vulnerable - unconfirmed. JNDI payload ({variant}) via "
                            f"POST body field '{field}' produced a definitive Log4j stack trace. "
                            f"This is NOT confirmed — a true positive requires an OOB callback "
                            f"server to observe the JNDI lookup reaching an external service."
                        ),
                        "remediation": (
    ...
    def _send_and_detect(
        ...
        Only reports findings based on response body analysis, NOT on
        connection errors, timeouts, or HTTP status codes alone.
        """
        try:
            if method == "GET":
    ...
    def _analyze_response(self, response) -> Optional[str]:
        ...
        Uses strict matching against REAL Java JNDI stack traces only.
        Do NOT report on HTTP 500, timeouts, or generic error strings.
        """
        indicators = []
        text = response.text
        ...
        for trace in java_traces:
            if trace in text:
                indicators.append(f"Definitive Log4j stack trace: '{trace}'")
