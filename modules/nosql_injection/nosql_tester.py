...
class NoSQLInjectionTester:
        ...
        for field in fields:
            for operator in self.MONGO_OPERATORS:
                ...
                try:
                    ...
                    result = self._detect_nosql_indicators(response, baseline)
                    if result:
                        severity = "info" if result["confidence"] == "low" else "high"
    ...
    def _detect_nosql_indicators(self, response, baseline: Dict) -> Optional[Dict]:
        """Detect NoSQL injection indicators in the response."""
        indicators = []

        # Check for MongoDB error messages (high confidence)
        response_text = response.text
        for error in self.ERROR_INDICATORS:
            if error in response_text:
                indicators.append(f"Error message found: '{error}'")

        # Check for authentication bypass (200 where baseline was 401/403, low confidence)
        ...
        if indicators:
            ...
            confidence = "high" if "Error message found" in evidence else "low"
