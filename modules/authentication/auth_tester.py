...
class AuthenticationTester:
    ...
    def _test_weak_passwords(self, url: str, context: Dict) -> List[Dict]:
        ...
        if not context.get('auth_endpoint', False):
            return vulnerabilities
        
        weak_passwords = [
            '123456', 'password', 'admin', '12345678',
            'qwerty', 'abc123', 'password123', '1234', 'admin123'
        ]
        ...
        for weak_pass in weak_passwords:
            try:
                response = self.http_client.post(
                    url,
                    data={'username': 'admin', 'password': weak_pass}
                )
                
                if response and response.status_code in [200, 302]:
                    has_session = 'Set-Cookie' in response.headers
                    
                    if has_session or 'success' in response.text.lower():
            ...
            for username, password in common_creds:
                try:
                    ...
                    if response and response.status_code == 302:
                        ...
                        if 'dashboard' in redirect_target.lower() or 'home' in redirect_target.lower():
    ...
    def _test_session_timeout(self, url: str, context: Dict) -> List[Dict]:
        ...
        try:
            ...
            if response and 'Set-Cookie' in response.headers:
                ...
                # Check for missing Max-Age or Expires
            ...
            if response:
                ...
                if jwt_match:
                    ...
                    try:
                        ...
                        header = base64.b64decode(padded).decode()
    ...
    def _test_mfa_bypass(self, url: str, context: Dict) -> List[Dict]:
        ...
        if not context.get('mfa_active', False):
            return vulnerabilities
        ...
        try:
            ...
            response = self.http_client.post(
                url,
                data={'code': test_code, 'mfa_code': test_code}
            )
            ...
            if response and response.status_code == 200:
                if test_code in response.text:
                    vulnerabilities.append({
                        ...
                        'severity': 'critical',
                        'url': url,
                        'evidence': f'MFA accepted unverified code: {test_code}',
                        'description': 'MFA code was accepted without verification',
