    ...
    def _check_signature_bypass(self, url: str) -> List[Dict]:
        ...
        try:
            ...
            if response and response.status_code in (200, 302):
                ...
                auth_indicators = [
                    'session', 'token', 'dashboard', 'profile', 'account',
                    'welcome', 'authenticated', 'samlresponse', 'relaystate',
                    'logged in', 'logout', 'my account'
                ]
                has_auth_indicator = any(ind in response_text_lower for ind in auth_indicators)
                no_error = 'error' not in response_text_lower and 'invalid' not in response_text_lower

                if has_auth_indicator and no_error:
                    vulnerabilities.append({
                        'type': 'SAML Signature Bypass (Unconfirmed)',
                        'severity': 'high',
                        'url': url,
                        'parameter': 'SAMLResponse',
                        'payload': 'Unsigned SAML assertion',
                        'evidence': (
                            f'Server returned {response.status_code} for unsigned SAML response. '
                            f'Response contained authentication indicators. '
                            f'NOTE: This is an UNCONFIRMED finding - the response may not reflect '
                            f'actual SAML processing. Manual verification is required.'
                        ),
                        'description': (
                            'The SAML endpoint accepted an unsigned assertion without returning an error, '
                            'and the response included authentication-related content. This MAY indicate '
                            'signature validation weakness, but could also mean the endpoint does not '
                            'process SAML at this URL. Manual verification is required.'
                        ),
                        'remediation': (
                            'Enforce signature validation on all SAML assertions. Reject unsigned or '
                            'improperly signed responses. Verify SAML endpoint configuration and ensure '
                            'ACS URLs only process signed assertions.'
                        )
                    })
        except Exception as e:
