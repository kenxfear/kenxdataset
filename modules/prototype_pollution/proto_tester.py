...
class PrototypePollutionTester:
    ...
    def _test_error_payloads(self, url: str, baseline: Optional[Dict]) -> List[Dict]:
        ...
        # Require canary persistence test to pass first
        persistence_vulns = self._test_pollution_persistence(url)
        if not persistence_vulns:
            return vulnerabilities
    ...
    def _analyze_response(
        ...
        """Analyze response for real prototype pollution indicators only."""
        indicators = []

        # Only REAL Node.js prototype pollution crash errors
        real_errors = [
            "Cannot set property",
        ...
        for error in real_errors:
            if error in response.text:
                indicators.append(f"Real prototype pollution crash: '{error}'")

        # 500 error ONLY if baseline didn't have 500
        if response.status_code == 500:
            if baseline is None or baseline["status_code"] != 500:
                indicators.append("Server error triggered by prototype payload")
