...
class OAuthTester:
    ...
    def _test_missing_state(self, endpoint: str) -> List[Dict]:
        ...
        # Only test if the URL already has required OAuth parameters
        required_oauth_params = ['response_type', 'client_id', 'redirect_uri']
        if not all(p in params for p in required_oauth_params):
            return vulnerabilities
        ...
        try:
            ...
            if response.status_code in (200, 302):
                ...
                oauth_flow_indicators = [
                    'code', 'authorization_code', 'consent', 'approve',
                    'callback', 'redirect_uri', 'grant_access', 'authorize',
                    'permission', 'allow access'
                ]
                has_oauth_flow = any(ind in response_text for ind in oauth_flow_indicators)

                if (has_oauth_flow and
                    "state" not in response_text and
                    "error" not in response_text[:500]):
                    vulnerabilities.append({
                        ...
                        "evidence": (
                            ...
                            f"Response contained OAuth flow indicators but no error about missing state."
                        ),
                        "description": (
    ...
    def _test_scope_escalation(self, endpoint: str) -> List[Dict]:
        ...
        # Only test if the URL already has a scope parameter
        ...
        for escalated_scope in self.ESCALATION_SCOPES:
            ...
            try:
                ...
                if response.status_code in (200, 302):
                    response_text = response.text.lower()
                    oauth_consent_indicators = [
                        'consent', 'authorize', 'approve', 'grant',
                        'allow', 'permission', 'scope', 'access'
                    ]
                    has_consent_screen = any(ind in response_text for ind in oauth_consent_indicators)

                    if (has_consent_screen and
                        "invalid_scope" not in response_text and
                        "error" not in response_text[:200]):
                        vulnerabilities.append({
                            ...
                            "evidence": (
                                f"Server returned consent/authorization page for elevated scope "
                                f"'{escalated_scope}' (original: '{original_scope}'). "
